...
class ReadingServiceTest {
    ...
    private final ReadingService service = new ReadingService(() -> List.of(
    ...
    void shouldReturnLatestReadingPerDeviceWhenDateRangeIsOmitted() {
        ...
        assertAll(
                ...
                () -> assertEquals(new BigDecimal("20"), result.getFirst().temperature().value()),
                () -> assertEquals(2L, result.get(1).deviceId()),
                () -> assertEquals(T12, result.get(1).measureTime()),
                () -> assertEquals(new BigDecimal("30"), result.get(1).temperature().value())
        );
    }
    ...
    void shouldReturnReadingsInsideInclusiveDateRange() {
        ...
        assertAll(
                ...
                () -> assertEquals(2L, result.get(2).deviceId()),
                () -> assertEquals(T12, result.get(2).measureTime())
        );
    }
}
